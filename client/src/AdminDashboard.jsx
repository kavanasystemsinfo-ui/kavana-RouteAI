...
const AdminDashboard = () => {
  ...
  if (showLogin) {
    return (
      <div style={{position: 'fixed', inset: 0, backgroundColor: '#000', zIndex: 20000, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px', fontFamily: "'Inter', sans-serif"}}>
        ...
        <h2 style={{color: '#f8cd00', fontSize: '16px', fontWeight: '900', letterSpacing: '1px', marginBottom: '8px'}}>TORRE DE CONTROL</h2>
  ...
  if (loading) return <div style={{backgroundColor: '#000', color: '#f8cd00', height: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', fontFamily: "'Inter', sans-serif", fontWeight: '900'}}>INICIANDO TORRE DE CONTROL...</div>;
    ...
    <div style={{backgroundColor: '#050505', minHeight: '100vh', color: '#fff', fontFamily: "'Inter', sans-serif", padding: '40px'}}>
      ...
      <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '40px'}}>
        <div style={{display: 'flex', alignItems: 'center', gap: '20px'}}>
          ...
          <div>
            <h1 style={{margin: 0, fontSize: '32px', fontWeight: '900', color: '#f8cd00', letterSpacing: '-1px'}}>KAVANA ROUTE AI</h1>
        ...
        <div style={{display: 'flex', gap: '16px', alignItems: 'center'}}>
          ...
          <button onClick={handleLogout} style={{backgroundColor: '#222', border: '1px solid #333', color: '#f8cd00', padding: '12px', borderRadius: '12px', cursor: 'pointer'}}>
            <LogOut size={20} />
          </button>
        </div>
      </div>
      ...
      {showSettings && (
        <div style={{backgroundColor: '#111', border: '1px solid #333', borderRadius: '16px', padding: '24px', marginBottom: '40px', display: 'flex', gap: '20px', alignItems: 'flex-end'}}>
          ...
          <button onClick={saveSettings} style={{backgroundColor: '#f8cd00', color: '#000', padding: '16px 32px', border: 'none', borderRadius: '12px', fontWeight: '900', fontSize: '14px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', height: 'fit-content'}}>
      ...
      {showDrivers && (
        <div style={{backgroundColor: '#111', border: '1px solid #333', borderRadius: '16px', padding: '24px', marginBottom: '40px'}}>
          <h3 style={{fontSize: '14px', fontWeight: '900', color: '#f8cd00', letterSpacing: '1px', marginBottom: '20px'}}>
          ...
          <form onSubmit={addDriver} style={{display: 'grid', gridTemplateColumns: '1fr 1fr 1fr auto auto', gap: '12px', marginBottom: '24px', alignItems: 'end'}}>
            ...
            <button type="submit" style={{backgroundColor: '#f8cd00', color: '#000', border: 'none', borderRadius: '8px', padding: '12px 20px', fontWeight: '900', fontSize: '13px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px'}}>
            ...
            {drivers.length === 0 ? (
              ...
              drivers.map(d => (
                <div key={d.id} style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 16px', borderBottom: '1px solid #222', backgroundColor: '#0a0a0a', marginBottom: '4px', borderRadius: '8px'}}>
                  <div style={{display: 'flex', gap: '16px', alignItems: 'center'}}>
                    <div style={{width: '40px', height: '40px', borderRadius: '50%', backgroundColor: d.active ? '#f8cd0020' : '#444', display: 'flex', alignItems: 'center', justifyContent: 'center', border: `2px solid ${d.active ? '#f8cd00' : '#666'}`}}>
                      <Truck size={20} color={d.active ? '#f8cd00' : '#666'} />
                    </div>
                    <div>
        ...
        {selectedDriver !== 'all' && (
          <div style={{display: 'flex', gap: '10px', alignItems: 'center'}}>
            ...
            <button onClick={() => fileInputRef.current?.click()} style={{
              backgroundColor: '#111', color: '#f8cd00', border: '1px solid #f8cd0040',
      ...
      <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px', marginBottom: '40px'}}>
        {[
          { label: 'COSTE TOTAL RUTA (OPEX)', value: `${currentOpex} €`, icon: <DollarSign size={24} color="#f8cd00" /> },
          ...
          <div key={idx} style={{backgroundColor: '#111', border: '1px solid #222', borderRadius: '16px', padding: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
            <div>
              ...
              <div style={{fontSize: '36px', fontWeight: '900', color: idx === 0 ? '#f8cd00' : '#fff'}}>{kpi.value}</div>
            </div>
              ...
              data.stops.map((stop, idx) => {
                ...
                return (
                  <tr key={idx} style={{borderBottom: '1px solid #222', backgroundColor: idx % 2 === 0 ? '#111' : '#141414'}}>
                    <td style={{padding: '20px', fontWeight: '900', color: '#f8cd00'}}>#{stop.stop_number}</td>
