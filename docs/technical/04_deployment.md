# Despliegue — KAVANA Route AI

## Arquitectura de despliegue (todo fuera del VPS)
| Componente | Donde vive | URL |
|---|---|---|
| Backend API (Node/Express, store JSON) | Render | https://routeai-api.onrender.com |
| Panel oficinas "Torre de Control" (React) | GitHub Pages (`gh-pages-admin`) | https://routeai.kavanasystems.com |
| App del repartidor (React PWA) | GitHub Pages (`gh-pages-admin`/`/app`) | https://routeai.kavanasystems.com/app |
...
- Variables obligatorias: `JWT_SECRET` (cadena ≥32 chars aleatoria),
  ...
  (def. github.io + routeai.kavanasystems.com).
...
(`routeai.json`) definido por `ROUTEFLEET_DB`.
Por defecto: `server/../../routeai.json` (dentro del código del repo).
...
- **Render Disk (plan de pago)**: montar volumen en `/data` y fijar
  `ROUTEFLEET_DB=/data/routeai.json`. Persistente entre reinicios.
...
- En repo → **Settings → Pages**:
  ...
  - Custom domain: **`routeai.kavanasystems.com`** → Save → **Enforce HTTPS**
...
```bash
# Backend vivo
curl https://routeai-api.onrender.com/api/settings
# Auth: sin token debe dar 401
curl -o /dev/null -w "%{http_code}\n" https://routeai-api.onrender.com/api/drivers
# CORS para panel
curl -s -i https://routeai-api.onrender.com/api/drivers \
  -H "Origin: https://routeai.kavanasystems.com" \
