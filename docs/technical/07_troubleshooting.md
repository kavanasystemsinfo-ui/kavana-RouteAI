# Troubleshooting — KAVANA Route AI
...
En Render FREE el filesystem es efímero: el archivo `routeai.json` se borra
...
## 2. Pantalla en blanco al abrir la app
...
## 3. El dominio routeai.kavanasystems.com da 404 tras un deploy
...
2. Reescribir `routeai.kavanasystems.com` → Save → Enforce HTTPS.
3. Esperar 1-2 min.

**Prevención:** el workflow `deploy-combined.yml` escribe `CNAME` en cada push. No debería volver a pasar.

---

## 4. Estrategia de distribución de la app (decisión 15/07/2026)

- **Canal único:** web en `/app` + "Añadir a pantalla de inicio"
...
- El APK (Capacitor) fue **descartado** como canal de distribución por su alto
  coste de mantenimiento. No se genera ni se distribuye (ADR-001, limpieza 2026-08-01).
