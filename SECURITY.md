...
- **API:** CORS configurado con lista fija, HTTPS forzado (Render + GitHub Pages), `trust proxy` para derivar la IP real del cliente en rate limiting
- **Sin secrets en el repositorio** — auditoría 2026-08-17: `JWT_SECRET` y `OFFICE_PIN` se rotaron y viven como secrets en Render (`render.yaml` usa `sync: false`). Detección automática de secretos en CI (gitleaks)
